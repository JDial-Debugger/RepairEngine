...
public class PsiNodeBuilder implements NodeBuilder {
	...
	@Override
	public AssignExpressionDelegate buildAssignExpression(ExpressionDelegate lhs, ExpressionDelegate rhs) {
		PsiAssignmentExpression assignExpression =
				(PsiAssignmentExpression) this.psiElementFactory.createExpressionFromText("a = b", null);

		PsiExpression psiLHS = this.elementExtractor.getDelegateElement(PsiExpression.class, lhs);
		PsiExpression psiRHS = this.elementExtractor.getDelegateElement(PsiExpression.class, rhs);
		assignExpression.getRExpression().replace(psiLHS);
		assignExpression.getLExpression().replace(psiRHS);
		return (AssignExpressionDelegate) assignExpression;
	}
